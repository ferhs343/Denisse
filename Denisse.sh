      ...
      if [[ ("$data" == "tcp.data" && ("$status" != "Unknown" &&
          ...
          for ((m=0;m<=${#src[@]} - 1;m++)); do
              ...
              if [ ${#dst[@]} -eq 1 ]; then
                  ...
                  for ((n=0;n<=${#ports[@]} - 1;n++)); do
                  	  port_str="${ports[$n]}"
                  	  for ((z=$n+1;z<=${#ports[@]} - 1;z++)); do
                  	  	next_port_str="${ports[$n]]}"
                  	  	if [ "$port_str" == "$next_port_str" ]; then
                  	  		multiple_src+=("${dst[$n]}")
                  	  		break; fi; done; done
                        	...
                        	if [ "$c" -gt 0 ]; then
                          	  ...
                          	  if [ "$data" == "tcp.data" ]; then tcp=1; else udp=1; fi
                            	alert="${src_type} ${HPS}"
                            	generate_results_tcp "${multiple_src[@]}" 
                              fi; fi; done; fi; done
}
...
function udp_hunt() {
	...
	tr ' ' '-' < ./Results/"$data" | awk '
			...
			END {
				for (s in seen) {
					split(seen[s], fields)
					print fields[1], fields[2], fields[3], fields[4], fields[6], fields[7], pkts[s], size[s], status[session]
				}
			}
	' > ./Results/.sample.data
