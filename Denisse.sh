...
function extract_pcap_data() {
    ...
    if [ "$proto" == "udp" ]; then
        ...
        tshark -r ./Pcaps/Trims/$pcap_file -Y "icmp" -T fields \
            ...
            -e "icmp.code" 2>/dev/null >./Results/tr_icmp_udp.data
    fi
    ...
    if [ "$proto" == "http" ]; then
        tshark -r ./Pcaps/Trims/$pcap_file -Y "http" -T fields \
            ...
            -e "http.file_data" 2>/dev/null >./Results/$proto.data
    fi
    ...
    if [ "$proto" == "dns" ]; then
        tshark -r ./Pcaps/Trims/$pcap_file -Y "dns" -T fields \
            ...
            -e "dns.aaaa" -e "dns.txt" 2>/dev/null >./Results/$proto.data
    fi
    ...
    for ((p = 0; p <= ${#sessions[@]} - 1; p++)); do
        ...
        if ((p == ${#sessions[@]} - 1)); then
            if [ "$data" == "tcp.data" ]; then
                ...
                filter_pcap+="udp.stream eq $stream"
            fi
            filter_json+="$stream"
        else
            if [ "$data" == "tcp.data" ]; then
                ...
                filter_pcap+="udp.stream eq $stream or "
            fi
            filter_json+="$stream|"
        fi
    done
